...
export const SignupView = ({ lang, t, setView }: SignupViewProps) => {
  return (
      ...
      <div className="w-full max-w-md" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        >
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl font-extrabold text-brand-primary">{t.auth.signupTitle}</h2>
          </div>

          <form className="space-y-5">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-muted px-2">{t.auth.fullName}</label>
              <input type="text" className="w-full bg-surface-base border border-border-subtle rounded-2xl px-6 py-4 outline-none focus:border-brand-primary transition-colors" placeholder={lang === 'ar' ? 'أحمد علي' : 'Ahmed Ali'} />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-text-muted px-2">{t.auth.password}</label>
          ...
          <div className="mt-10 pt-8 border-t border-border-subtle text-center">
            ...
            >
              {t.auth.haveAccount}{' '}
              <span className="text-brand-accent underline">{t.auth.ctaLogin}</span>
            </button>
          </div>
        </motion.div>
      </div>
