      ...
      <div className={`max-w-7xl mx-auto px-12 grid md:grid-cols-2 gap-16 items-center relative z-10 w-full flex-1 ${lang === 'ar' ? 'md:grid-flow-dense' : ''}`}>
        ...
        >
          ...
          <div className={`flex flex-col sm:flex-row gap-4 ${lang === 'ar' ? 'justify-start' : 'justify-start'}`}>
            ...
            <button 
              onClick={() => {
                const el = document.getElementById('reciters');
                el?.scrollIntoView({ behavior: 'smooth' });
              }}
              className="px-8 py-4 border border-brand-primary text-brand-primary rounded-xl font-bold text-lg hover:bg-white transition-all text-center"
            >
              {t.hero.ctaSecondary}
            </button>
          </div>
...
const Reciters = ({ t, lang }: { t: Translations, lang: Language }) => {
  const navigate = useNavigate();
  const recitersList = [
    { name: t.reciters.r1Name, specialty: t.reciters.r1Specialty, image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=200" },
    { name: t.reciters.r2Name, specialty: t.reciters.r2Specialty, image: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=200" },
    { name: t.reciters.r3Name, specialty: t.reciters.r3Specialty, image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=200" },
    { name: t.reciters.r4Name, specialty: t.reciters.r4Specialty, image: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=200" },
  ];

  return (
    <section id="reciters" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-12" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <div className="flex flex-col md:flex-row justify-between items-end gap-10 mb-20">
          <div className="max-w-xl text-right md:text-right">
            <h3 className="text-brand-accent font-bold uppercase tracking-widest text-xs mb-4">{t.reciters.badge}</h3>
            <h2 className="text-4xl md:text-5xl font-extrabold text-brand-primary mb-6">{t.reciters.title}</h2>
            <p className="text-text-muted text-lg font-light leading-relaxed">{t.reciters.subtitle}</p>
          </div>
          <button 
            onClick={() => navigate('/reciters')}
            className="px-8 py-4 border-2 border-brand-primary text-brand-primary rounded-xl font-bold hover:bg-brand-primary hover:text-white transition-all"
          >
            {t.reciters.viewAll}
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-6">
          {recitersList.map((r, i) => (
            <motion.div 
              key={i}
              whileHover={{ y: -5 }}
              className="bg-surface-base p-8 rounded-[2.5rem] border border-border-subtle text-center hover:border-brand-primary transition-all duration-300 group"
            >
              <div className="w-24 h-24 mx-auto mb-6 rounded-full overflow-hidden border-2 border-white shadow-lg group-hover:border-brand-primary transition-all">
                <img src={r.image} alt={r.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              </div>
              <h4 className="text-xl font-bold text-brand-primary mb-2">{r.name}</h4>
              <p className="text-[10px] text-text-muted font-bold mb-6 uppercase tracking-widest leading-loose h-10 flex items-center justify-center">
                {r.specialty}
              </p>
              <button className="w-full py-3 bg-white text-brand-primary rounded-xl font-bold text-sm shadow-sm hover:bg-brand-primary hover:text-white transition-all">
